...
//           $countts=$stmt->fetchcolumn();
  ...
  //   if($countts<=0)
  //   {
      $sql="INSERT INTO payment(user_id,price,status) VALUES(:user_id,:price,:status)"; 
       $stmt = $db->prepare($sql);
       if($stmt->execute(['user_id'=>$u_id,'price'=>$tot,'status'=>$s])){
...
echo '<tr>  
 ...
 {
  $s=0;
  $sql="INSERT INTO payment(amount,status,book_id) VALUES(:amount,:status,:book_id)"; 
  $stmt = $db->prepare($sql);
  if($stmt->execute(['amount'=>$price,'status'=>$s,'book_id'=>$b_id])){
     echo"<script>alert('Success');</script>";
     $_SESSION['status']=$s;
     header("Location:success.php");
  } 
